...
class Stripe():
	...
	def create_payment_method(self, doc, data):
		...
		try:
			...
			elif data.mode_of_payment.replace('New ', '') == 'ACH':  # TODO: replace with UI data collection / mandate / verification process
				response = stripe.PaymentMethod.create(
					type='us_bank_account',
					us_bank_account={
						'account_holder_type': frappe.get_value('Customer', doc.customer, 'customer_type').lower(),  # 'individual' or 'company'
	...
	def create_payment_intent(self, doc, data):
		...
		try:
			...
			if customer_id.get('message') == 'Success':
				...
				return {'message': 'Success', 'transaction_id': response.id, 'client_secret': response.client_secret}
			else:  # error creating customer profile
				return customer_id
		except Exception as e:
			try:
	...
	def process_credit_card(self, doc, data):
		...
		try:
			...
			if pm_response.get('message') == 'Success':
				...
				elif response.status in ['requires_action', 'requires_confirmation', 'requires_capture']:

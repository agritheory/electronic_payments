	...
	if eps.provider == "Authorize.Net":
		if data.mode_of_payment == "New Card" and data.save_data == "Charge now":
			...
			if payment_profile.retain == 0:
				frappe.db.set_value(
					doc.doctype, doc.name, "pre_authorization_token", payment_profile.payment_profile_id
				)
			else:
				frappe.db.set_value(
					"Customer", doc.customer, "electronic_payment_profile", payment_profile.customer_profile
				)
			return payment_profile
		else:

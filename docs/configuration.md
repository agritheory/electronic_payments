...
Authorize.net and Wise also require an endpoint, which is different for their testing sandbox calls and real production ones.
Authorize.net requires both an API Key and a Transaction Key, whereas Stripe requires only the API Key
...
- **Sending Provider:** (required) Authorize.net or Wise - this is the provider to **send** electronic payments and may be different than the one used to accept payments
- **Merchant ID:** (required for Wise) the Wise Profile ID for the company in Wise that will be associated with payment transfers. This is not easily discoverable in the Wise UI, for a list of options, leave this blank and fill in the other required fields (particularly the API endpoint and keys). When you click Save, it will call the API and list all the profiles associated with the given credentials
- **Endpoint:** (required) both Wise and Authorize.net have two distinct API endpoints - please refer to their respective documentation ([Wise API reference](https://docs.wise.com/api-docs/api-reference/environments), [Authorize.net API reference](https://developer.authorize.net/api/reference/index.html#gettingstarted-section-section-header)) for the most up-to-date values for endpoints.
...
- **API Key:** (required) the company's API key with the given provider
    - In testing: this should be the sandbox key for Authorize.net or Wise
    - In production: this should be the live production keys when being used in a production environment
- **Transaction Key:** (required for Authorize.net) the company's transaction key with Authorize.net
    - In testing: this should be the sandbox transaction key
    - In production: this should be the live transaction key
